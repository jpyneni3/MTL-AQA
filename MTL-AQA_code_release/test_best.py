...
test_logging_file_name = "test_best_" + str(current_run) + ".txt"
...
def train_phase(train_dataloader, optimizer, criterions, epoch):
    ...
    for data in train_dataloader:
        ...
        if with_caption:
            seq_probs, seq_preds = model_caption(clip_feats, true_captions, 'train')
...
def test_phase(test_dataloader):
    ...
    with torch.no_grad():
        ...
        for data in test_dataloader:
            ...
            if with_caption:
                seq_probs, seq_preds = model_caption(clip_feats, true_captions, 'test')
        ...
        test_logging_file.write("Generated Captions: {}".format(seq_preds) + "\n")
...
if __name__ == '__main__':
    ...
    saved_CNN_model = torch.load('c3davg_8_gru_attn_140_saved/model_CNN_99.pth')
    ...
    model_CNN.load_state_dict(saved_CNN_model)
    model_CNN = model_CNN.cuda()

    # loading our fc6 layer
    model_my_fc6 = my_fc6()
    saved_fc6_model = torch.load('c3davg_8_gru_attn_140_saved/model_my_fc6_99.pth')
    model_my_fc6.load_state_dict(saved_fc6_model)
    model_my_fc6.cuda()

    # loading our score regressor
    model_score_regressor = score_regressor()
    saved_model_score_regressor = torch.load('c3davg_8_gru_attn_140_saved/model_score_regressor_99.pth')
    model_score_regressor.load_state_dict(saved_model_score_regressor)
    ...
    if with_dive_classification:
        ...
        saved_model_dive_classifier = torch.load('c3davg_8_gru_attn_140_saved/model_dive_classifier_99.pth')
        model_dive_classifier.load_state_dict(saved_model_dive_classifier)
    ...
    if with_caption:
        ...
        model_caption = S2VTModel(vocab_size, max_cap_len, caption_lstm_dim_hidden,
                                  ...
                                  use_attention=False)
        saved_model_caption_pretrained_dict = torch.load('c3davg_8_gru_attn_140_saved/model_caption_99.pth')
        model_caption.load_state_dict(saved_model_caption_pretrained_dict)
